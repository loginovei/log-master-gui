...
- Двухэтапный поиск логов: мультивыбор шаблонов (Stage 1) → фильтрация записей по уровню, сервису, периоду и значениям параметров (Stage 2)
...
- Управление шаблонами: создание, редактирование, удаление с поддержкой нескольких языков и уровня логирования
...
- Фильтрация всех данных по приложению через глобальный селектор в шапке (с поиском)
...
```
Stage 1 — поиск шаблонов:
  Пользователь печатает → debounce 300ms → GET /api/templates/search?q=...
  Autocomplete: logCode — текст на текущем языке
  Пользователь выбирает один или несколько шаблонов (отображаются как теги)

Stage 2 — выборка записей:
  GET /api/logs?logCodes=A,B&level=...&from=...&to=...&argsQuery=...
  Фильтры: уровень, сервис, период (пресеты или произвольный диапазон), поиск по параметрам
  Если шаблоны не выбраны, но argsQuery задан — поиск по всем записям напрямую
```
...
Для таблицы последних логов все шаблоны загружаются один раз в `Record<logCode, LogTemplate>` через `useMemo` — рендеринг каждой строки O(1). Если шаблон не найден, отображается подсказка «не найден шаблон {logCode}».
...
| `isDark` | `boolean` | Активна ли тёмная тема |
| `toggleTheme` | функция | Переключить тему (сохраняется в `localStorage`) |
...
| `/logs` | `Logs.page` | Двухэтапный поиск и просмотр логов |
| `/templates` | `Templates.page` | Управление шаблонами (уровень, языки) |
...
├── components/
│   └── AppLayout.tsx   # Основной layout: навигация, шапка, селекторы, переключатель темы
├── context/
│   └── AppContext.tsx  # Глобальное состояние: приложение, язык, тема
├── hooks/
